...
For an integer $q\geq 0$, and filtering parameters $\{a_{-q},a_{-q+1},\ldots, a_0,a_1,\ldots,a_{q}\}$, we consider the following linear filter:
\[
\hat{m}_{t}=\sum_{k=-q}^{q}a_{k}X_{t-k}. 
\]
...
* For small $q$, $\hat{m}_{t}$ is more close to $m_t$, less bias, more variance;
...
This method is suitable for forecasting data with no clear trend or seasonal pattern. The function `ETS` function returns forecasts and other information for the simple exponential smoothing forecasts applied to `y`.
...
``````{r pred4, warning=FALSE, message=FALSE, fig.height=4, fig.width=7, fig.align = "center", fig.cap="Two weeks ahead forecast of the daily death count for Florida using four different methods."}
# Fit the models
...
We can use the `hilo()` function to convert the forecast distributions into intervals. By default, 80% and 95% prediction intervals are returned. We can use `level` argument to control coverage.
...
Instead of checking to see whether each sample autocorrelation falls inside the bounds defined in \@ref(fig:resets) above, it is also possible to consider the portmanteau test such as the Ljung–Box test. The hypothesis of iid data is then rejected at level $\alpha$ if the p-value is smaller than $\alpha$.
...
\begin{align*}
\text{MAE}  &= \text{mean}(|e_{n+h}|)~~~~~
\text{MSE}  = \text{mean}(e_{n+h}^2) \\
\text{RMSE} &= \sqrt{\text{mean}(e_{n+h}^2)} ~~~~
\text{MAPE} = 100\text{mean}(|e_{n+h}|/ |Y_{n+h}|)\\
\text{MASE} &= \text{mean}(|e_{n+h}|/Q)~~~Q = (n-1)^{-1}\sum_{t=2}^n |Y_t-Y_{t-1}|
\end{align*}
...
Reg_fc %>% 
  ...
  autolayer(filter_index(dplyr::select(Florida.ts, Y.Death), 
                         "2020-11-28" ~ .), color = "black") +
  labs(y = "Death count", 
       title = "Exponential Smoothing with 
...
The time series $\{X_t\}$ is said to be an autoregressive process of order $p$ (or AR( p)) if
$\theta(z) \equiv 1$, and a moving-average process of order $q$ (or MA(q)) if $\phi(z) \equiv 1$.
...
<!-- \] -->
<!-- for any $a\in \mathbb‹€´π{R}$. So, the same process $\{X_t\}$ would satisfy different ARMA models.  -->
...
* If the RHS of formula is left blank, the default search space is given by `pdq() + PDQ()`: that is, a model with candidate seasonal and nonseasonal terms, but no exogenous regressors.
...
```
     ...
     + The `period` argument is used in `PDQ` to specify the periodic nature of the seasonality.

Below, we fit different ARIMA models for the daily death count, and conduct the two weeks ahead forecast.
...
To compare how well the models fit the data, we can consider some common accuracy measures. It seems that on the training set the ETS model out-performs ARIMA for the series where travellers are on holiday, business, and visiting friends and relatives. 
