...
In the above, two important arguments are:
...
However, this grid-search approach won't perform well when we have more than one parameter. For example, we would like to assume that $\beta$, $\gamma$ and $I_0$ are all unknown, and we would like to estimate them simultaneously. In that case, we can use the `optim` function instead.
