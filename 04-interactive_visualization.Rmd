...
Next, we will apply the `geom_bar` and `coord_polar` functions together with `ggplot` to display the pie chart; see Figure\@(fig:pie0).  
...
2. Redraw the above plots in (b), (c) and (d) for the death count using the `ggplotly()` function. Save the above plots as an HTML file, and save all the dependency files externally into a single directory using `htmlwidgets::saveWidget()` with `selfcontained = FALSE`.
