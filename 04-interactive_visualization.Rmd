...
We can also make pie charts in R using `plotly` using the function `add_pie`.  To draw the pie chart, we upload the `features.state` from the `slid` R package, and the dataset contains four variables: State, Region, Division and pop. We are interested in the composition of the population in each region. 

```{r pie1, out.width = "50%", echo=FALSE, fig.align = "center", fig.cap="A simple pie chart for population in different regions."}
library(slid)
data(features.state)
...
* `p`: a plotly object;
* `frame`:  the amount of time between frames;
* `transition = 500` frame: the duration of the smooth transition between frames;
* `easing = "linear"`: the type of transition easing;
* `redraw = TRUE`: trigger a redraw of the plot at the completion of the transition or not;
* `mode = "immediate"`: describe how a new animate call interacts with currently-running animations.

Figure \@ref(fig:animate2) illustrates the same plots as Figure \@ref(fig:animate1), but doubles the amount of time between frames, uses linear transition easing, places the animation buttons closer to the slider, and modifies the default `currentvalue.prefix` settings for the slider.

```{r animate2, fig.show='animate', message=FALSE, warning=FALSE, fig.cap="Modifying animation defaults with `animation_opts()`, `animation_button()`, and `animation_slider()`."}
base <- state.long.DEC %>%
...
2. Redraw the above plots for the death count using the `ggplotly()` function. 
